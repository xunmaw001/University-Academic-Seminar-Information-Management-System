<template>
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="学术类型" prop="xueshuleixing">
					<el-input v-model="ruleForm.xueshuleixing" placeholder="学术类型" clearable  :readonly="ro.xueshuleixing"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="学术类型" prop="xueshuleixing">
					<el-input v-model="ruleForm.xueshuleixing" placeholder="学术类型" readonly></el-input>
				</el-form-item>
			</template>
...
export default {
	data() {
		...
		return {
			...
			rules: {
				xueshuleixing: [
					{ required: true, message: '学术类型不能为空', trigger: 'blur' },
				],
			}
		};
	},
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xueshuleixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xueshuleixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
